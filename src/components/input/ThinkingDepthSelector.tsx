...
const MAX_LEVEL = THINKING_LEVELS.length - 1;
...
export const ThinkingDepthSelector = memo(function ThinkingDepthSelector({
  ...
  const measureTrack = useCallback(() => {
    ...
    // clientWidth/clientLeft describe the padding box, which is what
    // `left`-positioned children are relative to. Using the border-box
    // rect here makes the endpoints asymmetric and clips the thumb.
  ...
  const drawCanvas = useCallback((time: number, dt: number) => {
    ...
    // Effect intensity grows with thinking depth: Low is calm, High is lively.
    const depth = clamp(posRef.current / MAX_LEVEL);
    const depthScale = 0.55 + 0.45 * depth;

    /* Leading-edge soft glow */
    ...
    /* Pixel/ember cell field — ignites from the low end toward the thumb,
       with cells brightening as they approach the leading edge */
    const cell = 6;
    const gap = 1.4;
    ...
    /* Ember / star stream: ignites from low end toward the thumb */
    ...
    /* Comet sparks traveling along the whole stream, denser near the edge */
    if (particles) {
      for (let i = 0; i < 8; i += 1) {
  ...
  const applyMagnet = useCallback((nextValue: number) => {
    ...
    // Deliberately weak magnet: the thumb follows the pointer freely and
    // only sticks when it is very close to a level.
  ...
  const setupCanvas = useCallback(() => {
    ...
    if (!particlesRef.current) {
      particlesRef.current = Array.from({ length: 140 }, makeParticle);
    }
    layoutDots();
    updateThumb(posRef.current);
  ...
  // One-shot ignition when the panel opens, so the depth-based effect is
  // visible immediately (Low = small pulse, High = full stream).
  useEffect(() => {
