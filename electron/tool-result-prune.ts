...
function isCritical(obj: Record<string, unknown>, plan: any): boolean {
...
function summarize(content: string, plan: any, config: PruneConfig): string {
  ...
  if (typeof obj.file_path === 'string' && typeof obj.content === 'string') {
    ...
    return JSON.stringify({
      ...
      content: truncate(obj.content as string, critical ? maxCritical : Math.min(maxKeep, 800)),
...
export function pruneToolResults(messages: any[], plan: any, config: PruneConfig = {}): PruneResult {
