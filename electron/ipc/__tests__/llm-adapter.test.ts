...
describe('llm-adapter registry', () => {
  ...
  it('throws for an unknown explicit adapter id', async () => {
    await expect(invokeLlm({ ...baseParams(), adapter: 'no-such-provider' })).rejects.toThrow('未注册的 LLM 适配器');
  });
});
...
describe('invokeDeepSeekOpenAI — 流式解析', () => {
  ...
  it('流式过滤器剥离 <FINAL_ANSWER> 标记', async () => {
    vi.mocked(axios.post).mockResolvedValue(
      openaiBody(['data: {"choices":[{"delta":{"content":"done <FINAL_ANSWER>"}}]}\n\n', 'data: [DONE]\n\n']) as any,
    );
  ...
  it('length / 其他 finish_reason 映射', async () => {
    vi.mocked(axios.post).mockResolvedValueOnce(
      openaiBody(['data: {"choices":[{"finish_reason":"length"}]}\n\n', 'data: [DONE]\n\n']) as any,
    );
    expect((await llmClientInvoke(baseParams()))!.completionStopReason).toBe('max_tokens');

    vi.mocked(axios.post).mockResolvedValueOnce(
      openaiBody(['data: {"choices":[{"finish_reason":"content_filter"}]}\n\n', 'data: [DONE]\n\n']) as any,
    );
