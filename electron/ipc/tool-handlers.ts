...
function spawnBashChild(
  ...
  if (ctx.toolCallId) {
    abortRegistry.set(ctx.toolCallId, {
      abort: () => {
        userAborted = true;
        child.kill('SIGTERM');
...
function runBashBackground(command: string, workdir: string, ctx: ToolContext): ToolResult {
  ...
  abortRegistry.set(taskId, { abort: abortBackground });
  if (ctx.toolCallId) abortRegistry.set(ctx.toolCallId, { abort: abortBackground });
  let stdout = '';
  let stderr = '';
  const stdoutDecoder = createOutputDecoder();
  const stderrDecoder = createOutputDecoder();
  ...
  const done = (exitCode: number | null, error?: string) => {
    ...
    if (ctx.toolCallId) abortRegistry.delete(ctx.toolCallId);
