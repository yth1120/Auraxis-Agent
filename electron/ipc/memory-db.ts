 ...
 *
 * Provenance model (Eywa / MAP-Graph):
 *   Evidence（不可变源）─► Signal（类型化索引）─► Belief（可修订信念）
 *   信念必须引用 ≥1 条证据；旧记忆迁移为 legacy=1 的信念。
 */
...
import { openSqlite, sqliteAvailable } from '../session-projection-cache';
...
class JsonBackend implements MemoryBackend {
